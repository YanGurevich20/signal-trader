  ...
  try {
    while (true) {
      ...
      for (const [index, pool] of pools.entries()) {
        ...
        if (twitterHandle) {
          try {
            const userInfo = await getOrInsertUser(twitterHandle);
            if (userInfo.follower_count > 10000) {
              console.log("--------------------------------");
              console.log(
                `${new Date().toLocaleTimeString("en-GB")} - ${index + 1}/${pools.length} - ${tokenInfo.attributes.symbol} has ${userInfo.followers} followers. Link: https://www.geckoterminal.com/solana/pools/${poolAddress}.`,
              );
              console.log(
                `Existing pools: ${existingPools.length}. Pool created ${hours}h ${minutes}m ${seconds}s ago`,
              );
              console.log("--------------------------------");
            }
          } catch {
